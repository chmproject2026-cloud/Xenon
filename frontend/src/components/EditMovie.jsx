...
const EditMovie = ({ movie, fetchMovies, trigger }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(`/movies/${movie._id}`, finalData, { // http://localhost:5000/api/movies/${movie._id}
          ...
          <form onSubmit={handleUpdate} className="grid grid-cols-2 gap-x-8 gap-y-10">
            ...
            <div className="col-span-2 flex items-center gap-6 mt-6 pt-10 border-t border-white/5">
              <button
                type="submit"
                disabled={isSyncing}
                className={`flex-1 h-16 rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all border-none flex items-center justify-center gap-3 shadow-2xl ${isSyncing
